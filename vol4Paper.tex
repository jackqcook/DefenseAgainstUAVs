...
Autonomous drone on drone tracking presents a complex control problem in which a defensive quadrotor must handle nonlinear flight dynamics and noisy sensor measurements in real time. While a classical feedback controller performs well near a single equilibrium, we want our defensive quadrotor to engage in aggressive tracking over rapidly changing flight conditions where a single linearization is insufficient.
...
Because the sensors give a noisy picture of the quadrotor, we need a more reliable state estimate than a single raw snapshot. We use the Kalman filter to propogate a forward prediction using the linearized model, then nudge that prediction toward what the sensors report. The quantity we use in the controller is the filtered estimate $\hat{\mathbf{x}}_k$.

In our simulation every state component is measured directly, so
\[
C = I_{12},
\qquad
...
\]
where $\mathbf{v}_k$ is measurement noise. The true plant state is also disturbed each step by process noise $\mathbf{w}_k \sim \mathcal{N}(\mathbf{0},\,V)$ (added in the simulation loop after the nonlinear integration step). We use diagonal covariances
\[
V = 10^{-7}\, I_{12},
\qquad
W = 10^{-3}\, I_{12},
...
For each trim we reuse the same discretized matrices $A_d$ and $B_d$ as in the linearization section. The notebook computes the Kalman gain $L$ from the discrete algebraic Riccati equation, then, in deviation form about the scheduled equilibrium $(\mathbf{x}_{\mathrm{eq}},\mathbf{u}_{\mathrm{eq}})$, predicts and corrects:
\begin{align*}
...
With a state estimate $\hat{\mathbf{x}}_k$ in hand from the Kalman filter, we need a control law that drives the drone toward the target. The Linear Quadratic Regulator finds the gain matrix $K$ that minimizes the infinite-horizon cost
\[
J = \sum_{k=0}^{\infty} \left[(\mathbf{x}_k - \mathbf{x}_d)^\top Q\,(\mathbf{x}_k - \mathbf{x}_d) + \mathbf{u}_k^\top R\,\mathbf{u}_k\right],
\]
where $\mathbf{x}_d$ is the desired state and $Q$, $R$ are tuning weights. $Q$ penalizes deviation from the goal---maintaining the tracker exactly 1 meter directly behind the target---and $R$ penalizes control effort. We solve for $K$ at each of the 27 operating points offline via the discrete algebraic Riccati equation, storing the result in the schedule bank alongside $L$.
...
\]
where $\mathbf{x}_d$ encodes the 1-meter-behind tracking objective. Because the scheduler continuously re-selects the nearest local model, the controller sees a linearization that is always a good approximation to the true nonlinear dynamics---enabling near-optimal performance across the full flight envelope without any online optimization.
