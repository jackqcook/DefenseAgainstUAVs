...
While the gain-scheduled LQG framework demonstrates strong performance in simulation, limitations remain. The controller relies on locally linear approximations, which may degrade under highly aggressive maneuvers or dynamics far from the predefined operating points. From an ethical perspective, the underlying techniques used in this paper are directly applicable to autonomous aerial systems with potential military use. This highlights the importance of considering responsible deployment and adherence to regulatory frameworks when developing and applying such control systems.

Future work could extend this approach by incorporating rotor saturation constraints more explicitly, exploring nonlinear estimation techniques such as the extended or unscented Kalman filter, and validating the controller on a physical hardware simulator. Increasing the density of the operating-point grid, or replacing the nearest-neighbor scheduler with a smooth interpolation scheme, could further reduce transient switching artifacts and improve performance during rapid flight-regime changes. More broadly, this approach illustrates how classical linear control techniques can be extended to complex nonlinear autonomous systems through structured model scheduling.
